...
config = {
    ...
    'lr': 1e-5,
    'epochs': 265,
    'properties': sorted(['affinity','logps']),
}
...
for b in target_props['affinity']:
    for c in target_props['logp']:
        key = f"{b}-{c}"
        t = torch.Tensor([
            affinity_scaler.transform([[b]]).flatten()[0],
            logp_scaler.transform([[c]]).flatten()[0]
            ])
        queries.append(key)
...
# %%
data = []
columns = ['Target Affinity','Target LogP','Predicted LogP','Predicted QED' ,'Predicted SAS','Predicted TPSA', 'key']
for key in results:
    keys = key.split('-')
    target_affinity = keys[1]
    target_logp = keys[2]
    for i in range(len(results[key][0])):
        data.append([target_affinity,target_logp, results[key][0][i], results[key][1][i], results[key][2][i], results[key][3][i], key])
