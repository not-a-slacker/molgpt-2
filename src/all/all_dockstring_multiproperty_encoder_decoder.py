...
# %%
# %%
stats = pd.DataFrame({ "LogP_pred":logps, "tpsa_pred":tpsas, "QED_pred":qeds, "SAS_pred":sas})
...
for a in target_props['affinity']:
    for b in target_props['logps']:
        for c in target_props['qeds']: 
            for d in target_props['sas']:
                for e in target_props['tpsas']:
                    key = str(a)+'_'+str(b)+'_'+str(c) + '_' + str(d) + '_' + str(e)
                    t = torch.Tensor([
...
for key, v in zip(queries, property_vectors):
    ...
    end_time = time.time()
    print(f"Time taken for {key}: {end_time - start_time} seconds")
...
with open('../checkpoints/' + config['run_name'] +'/generated_molecules.pkl', 'wb') as f:
    pickle.dump(results_dict, f)

# # %%
# data = []
# columns = ['Target TPSA','Target LogP','Target SAS','Target QED', 'Predicted LogP','Predicted QED' ,'Predicted SAS','Predicted TPSA', 'key']
# for key in results:
#     keys = key.split('_')
#     target_tpsa = float(keys[4])
#     target_logp = float(keys[1])
#     target_qed = float(keys[2])
#     target_sas = float(keys[3])
#     for i in range(len(results[key][0])):
#         data.append([target_tpsa,target_logp,target_sas,target_qed, results[key][0][i], results[key][1][i], results[key][2][i], results[key][3][i], key])
...
# a=sns.kdeplot(data=data_df, x="Predicted LogP",hue="Target LogP", fill=True,alpha=.5, linewidth=1, bw_adjust=2)
...
# # %%
# sns.kdeplot(data=data_df, x="Predicted SAS",hue="Target SAS", fill=True,alpha=.5, linewidth=1, bw_adjust=2)
# a=sns.kdeplot(data=data_df, x="Predicted SAS",hue="Target SAS", fill=True,alpha=.5, linewidth=1, bw_adjust=2)
# (a.get_figure()).savefig(os.path.join(results_dir, 'sas_targeted.png')) 


# # %%
# mae_var_df_tpsa = data_df.groupby('Target TPSA').apply(
#     lambda g: pd.Series({
#         'MAE': np.mean(np.abs(g['Predicted TPSA'].astype(float) - float(g['Target TPSA'].iloc[0])),
#         ),
#         'Variance': np.var(g['Predicted TPSA'].astype(float))
#     })
# ).reset_index()

# print(mae_var_df_tpsa)
# mae_var_df_tpsa.to_csv(os.path.join(results_dir, f'tpsa_mae_variance.csv'))

# # %%
# mae_var_df_logp = data_df.groupby('Target LogP').apply(
#     lambda g: pd.Series({
...
#     })
# ).reset_index()
# print(mae_var_df_logp)
# mae_var_df_logp.to_csv(os.path.join(results_dir, f'tpsa_mae_variance.csv'))

# # %%
# mae_var_df_qed = data_df.groupby('Target QED').apply(
#     lambda g: pd.Series({
...
#     })
# ).reset_index()
# print(mae_var_df_qed)
# mae_var_df_qed.to_csv(os.path.join(results_dir, f'qed_mae_variance.csv'))

# # %%
# mae_var_df_sas = data_df.groupby('Target SAS').apply(
#     lambda g: pd.Series({
#         'MAE': np.mean(np.abs(g['Predicted SAS'].astype(float) - float(g['Target SAS'].iloc[0])),
#         ),
#         'Variance': np.var(g['Predicted SAS'].astype(float))
#     })
# ).reset_index() 
# print(mae_var_df_sas)
# mae_var_df_sas.to_csv(os.path.join(results_dir, f'sas_mae_variance.csv'))


# # %%
# data_df['TPSA-SAS'] = data_df['Target TPSA'].astype(str) +'-' +data_df['Target SAS'].astype(str)
# # %%
# filtered_df = data_df.query("`Target TPSA` in [40,100] and `Target SAS` in [2,4]")
# ax = sns.kdeplot(data=filtered_df, x="Predicted TPSA", y="Predicted SAS", hue="TPSA-SAS", fill=True, alpha=0.7, bw_adjust=1.5)
# (ax.get_figure()).savefig(os.path.join(results_dir, 'tpsa_sas_targeted.png'))

# # %%
# data_df['TPSA-LOGP'] = data_df['Target TPSA'].astype(str) +'-' +data_df['Target LogP'].astype(str)


# # %%
# filtered_df = data_df.query("`Target TPSA` in [40,100] and `Target LogP` in [1,5]")
# ax = sns.kdeplot(data=filtered_df, x="Predicted TPSA", y="Predicted LogP", hue="TPSA-LOGP", fill=True, alpha=0.7, bw_adjust=1.5)
