...
config['run_name'] = "LCK_DOCKSTRING_FAST_ACTUAL_"+ "_".join(prop for prop in config['model_properties'])
print(config)
...
def run(config,preference_dataset):
    ...
    if config['ipo']:
        config['run_name'] = f"IPO_Finetuning_BETA_{config['beta']}_epochs_{config['epochs']}_LCK_DOCKSTRING_FAST_ACTUAL_{model_props_str}{dpo_tag}"
    else:
        config['run_name'] = f"DPO_Finetuning_BETA_{config['beta']}_epochs_{config['epochs']}_LCK_DOCKSTRING_FAST_ACTUAL_{model_props_str}{dpo_tag}"
    ...
    print(f"python analyze_generated_molecules.py --checkpoint_dir {checkpoint_dir} --properties {' '.join(config['model_properties'])}")
    print("="*50 + "\n")
...
# Use model_properties for loading the base model
config['run_name'] = "LCK_DOCKSTRING_FAST_ACTUAL_"+ "_".join(prop for prop in config['model_properties'])
config['beta'] = BETA
...
# Determine which preference dataset to use based on preference_properties
if config['preference_properties'] == ['affinity']:
    # Use affinity-only preference dataset
    # Load from the full model's checkpoint directory
    base_checkpoint = "LCK_DOCKSTRING_FAST_ACTUAL_affinity_logps_qeds_sas_tpsas"
    preference_file = f'../checkpoints/{base_checkpoint}/PreferenceData_affinity.pkl'
    print(f"Loading affinity-only preference data from: {preference_file}")
else:
    # Use multi-property preference dataset
    base_checkpoint = "LCK_DOCKSTRING_FAST_ACTUAL_affinity_logps_qeds_sas_tpsas"
    preference_file = f'../checkpoints/{base_checkpoint}/PreferenceData.pkl'
    print(f"Loading multi-property preference data from: {preference_file}")

with open(preference_file, 'rb') as f:
    preference_data = pickle.load(f)

# Use preference_properties for the preference dataset (not model_properties)
...
# config['run_name'] = "EGFR_Decoder_affinity_logps_qeds_tpsa"
...
#     # results[key] = [logps, tpsa, qeds, samples]
