...
def run(config):
    ...
    print("No of GPUs available", num_gpus, flush=True)
...
# run(config)
# end_time = time.time()
# print(f"Total training time: {end_time - start_time} seconds",flush=True)
...
sns.pairplot(stats)
plt.savefig(os.path.join(results_dir, 'logp_pairplot.png'))
plt.close()
# %%
target_props = {}
...
plt.savefig(os.path.join(results_dir, 'logp_targeted.png'))
plt.close()
