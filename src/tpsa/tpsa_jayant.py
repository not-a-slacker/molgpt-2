...
def run(config):
    ...
    path_dir = '../checkpoints/'+ config['run_name'] + '/best_model'
...
def load_model(config):
    path_dir = '../checkpoints/'+ config['run_name']
    model_path = path_dir + '/' + 'best_model' + '.pt'
    model = SmileDecoder(d_model=config['d_model'], 
                   n_heads=config['n_heads'], 
                   n_layers=config['n_layers'], 
                   vocab=smiles_vocab, 
...
og_tpsa = og_props[:,0]
# %%
results_dir = '../checkpoints/' + config['run_name'] + '/results1'
try:
...
queries = []
property_vectors = []
for b in target_props['tpsa']:

    key = str(b)
    t = torch.Tensor([
        tpsas_scaler.transform([[b]]).flatten()[0]
        ])
    queries.append(key)
...
# %%
data = []
columns = ['Target TPSA', 'Predicted LogP','Predicted QED' ,'Predicted SAS','Predicted TPSA', 'key']
for key in results:
    target_tpsa = key
    for i in range(len(results[key][0])):
        data.append([target_tpsa, results[key][0][i], results[key][1][i], results[key][2][i], results[key][3][i], key])
