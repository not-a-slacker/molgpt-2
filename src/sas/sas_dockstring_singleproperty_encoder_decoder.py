...
queries = []
property_vectors = []
for b in target_props['sas']:

    key = str(b)
    t = torch.Tensor([
        sas_scaler.transform([[b]]).flatten()[0]
        ])
    queries.append(key)
...
# %%
data = []
columns = ['Target SAS', 'Predicted LogP','Predicted QED' ,'Predicted SAS','Predicted TPSA', 'key']
for key in results:
    target_sas = key
    for i in range(len(results[key][0])):
        data.append([target_sas, results[key][0][i], results[key][1][i], results[key][2][i], results[key][3][i], key])
...
# %%
# Calculate MAE and variance for SAS
