...
config = {
    ...
    'lr': 1e-5,
    'epochs': 125,
    'properties': sorted(['affinity','sas']),
}
...
plt.ylabel('Density')
plt.title('SAS Score Distribution Comparison')
...
target_props['affinity'] = [-9,-6]

# %%
queries = []
property_vectors = []
for b in target_props['affinity']:
    for c in target_props['sas']:
        key = f"{b}_{c}"
        t = torch.Tensor([
            affinity_scaler.transform([[b]]).flatten()[0],
            sas_scaler.transform([[c]]).flatten()[0]
            ])
        queries.append(key)
...
# %%
data = []
columns = ['Target Affinity','Target SAS','Predicted LogP','Predicted QED' ,'Predicted SAS','Predicted TPSA', 'key']
for key in results:
    keys = key.split('_')
    target_affinity = keys[0]
    target_sas = keys[1]
    for i in range(len(results[key][0])):
        data.append([target_affinity,target_sas, results[key][0][i], results[key][1][i], results[key][2][i], results[key][3][i], key])
