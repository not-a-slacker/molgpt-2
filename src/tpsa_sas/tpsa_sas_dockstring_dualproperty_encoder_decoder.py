...
# %%
# columns = ['smiles', 'affinity', 'logps', 'qeds', 'tpsas', 'sas','split']
config = {
    'batch_size' : 128,
    'd_model': 512,
    'n_heads': 8,
...
# %%
idx_tpsa = config['properties'].index('tpsas')
idx_sas  = config['properties'].index('sas')

og_tpsa = og_props[:, idx_tpsa]
og_sas = og_props[:, idx_sas]
# %%
...
queries = []
property_vectors = []
for sas_val in target_props['sas']:
    for tpsa_val in target_props['tpsa']:
        key = f"{sas_val}-{tpsa_val}"
        val_map = {}
        val_map['sas'] = sas_scaler.transform([[sas_val]]).flatten()[0]
        val_map['tpsas'] = tpsas_scaler.transform([[tpsa_val]]).flatten()[0]
        vec = torch.Tensor([val_map[p] for p in config['properties']])
        queries.append(key)
...
# %%
data = []
columns = ['Target TPSA','Target SAS','Predicted LogP','Predicted QED' ,'Predicted SAS','Predicted TPSA', 'key']
for key in results:
    keys = key.split('-')
    target_tpsa = keys[1]
    target_sas = keys[0]
    for i in range(len(results[key][0])):
        data.append([target_tpsa,target_sas, results[key][0][i], results[key][1][i], results[key][2][i], results[key][3][i], key])
...
# %%
mae_var_df = data_df.groupby('Target SAS').apply(
    lambda g: pd.Series({
...
print(mae_var_df)
mae_var_df.to_csv(os.path.join(results_dir, f'sas_mae_variance.csv'))
