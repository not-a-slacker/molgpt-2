...
with open('/home2/bhuvan.kapur/MolGPT2.0/checkpoints/LCK_DOCKSTRING_FAST_ACTUAL_affinity_logps_qeds_sas_tpsas/results.pkl', 'rb') as f:
...
for key in results:
    ...
    DIST = results[key][3]
    DIST = np.array(DIST)
    DIST = DIST[:DIST.shape[0] - (DIST.shape[0] % K)]
...
pickle.dump(data, open('/home2/bhuvan.kapur/MolGPT2.0/checkpoints/LCK_DOCKSTRING_FAST_ACTUAL_affinity_logps_qeds_sas_tpsas/affinity_comparision.pkl', 'wb'))
    
for key in data:
    plt.hist(data[key], bins=10, alpha=0.5, label="-" + key.split('-')[1] + " (kcal/mol)")
    
plt.legend()
plt.savefig('/home2/bhuvan.kapur/MolGPT2.0/checkpoints/LCK_DOCKSTRING_FAST_ACTUAL_affinity_logps_qeds_sas_tpsas/affinity_comparision.png')
# %%
