...
config = {
    ...
    'lr': 1e-5,
    'epochs': 46,
    'properties': sorted(['affinity','tpsas']),
}
...
# %%
data = []
columns = ['Target Affinity','Target TPSA','Predicted LogP','Predicted QED' ,'Predicted SAS','Predicted TPSA', 'key']
for key in results:
    keys = key.split('_')
    target_affinity = keys[0]
    target_tpsa = keys[1]
    for i in range(len(results[key][0])):
        data.append([target_affinity,target_tpsa, results[key][0][i], results[key][1][i], results[key][2][i], results[key][3][i], key])
