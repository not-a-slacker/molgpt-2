...
for b in target_props['logp']:
    for c in target_props['tpsa']:
        key = f"{b}-{c}"
        t = torch.Tensor([
            logp_scaler.transform([[b]]).flatten()[0],
            tpsas_scaler.transform([[c]]).flatten()[0]
            ])
        queries.append(key)
...
# %%
data = []
columns = ['Target TPSA','Target LogP','Predicted LogP','Predicted QED' ,'Predicted SAS','Predicted TPSA', 'key']
for key in results:
    keys = key.split('-')
    target_tpsa = keys[1]
    target_logp = keys[0]
    for i in range(len(results[key][0])):
        data.append([target_tpsa,target_logp, results[key][0][i], results[key][1][i], results[key][2][i], results[key][3][i], key])
...
# %%
sns.histplot(data=data_df, x="Predicted TPSA",hue="Target TPSA", stat="density", common_norm=False, bins=50, alpha=0.5)
a = sns.histplot(data=data_df, x="Predicted TPSA",hue="Target TPSA", stat="density", common_norm=False, bins=50, alpha=0.5)
(a.get_figure()).savefig(os.path.join(results_dir, 'tpsa_targeted_hist.png'))
...
# %%
sns.histplot(data=data_df, x="Predicted LogP",hue="Target LogP", stat="count", common_norm=False, bins=50, alpha=0.5)
a = sns.histplot(data=data_df, x="Predicted LogP",hue="Target LogP", stat="count", common_norm=False, bins=50, alpha=0.5)
(a.get_figure()).savefig(os.path.join(results_dir, 'logp_targeted_hist.png'))
...
# %%
data_df['TPSA-LogP'] = data_df['Target TPSA'] +'-' +data_df['Target LogP']
ax = sns.histplot(data=data_df, x="Predicted TPSA", y="Predicted LogP", hue="TPSA-LogP", stat="count", common_norm=False, bins=50, alpha=0.5)
(ax.get_figure()).savefig(os.path.join(results_dir, 'tpsa_logp_targeted_hist.png'))
# %%
mae_var_df = data_df.groupby('Target TPSA').apply(
    lambda g: pd.Series({
...
print(mae_var_df)
mae_var_df.to_csv(os.path.join(results_dir, f'tpsa_mae_variance.csv'))
...
print(mae_var_df)
mae_var_df.to_csv(os.path.join(results_dir, f'logp_mae_variance.csv'))
